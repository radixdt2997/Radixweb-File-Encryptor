...
export const Sender = ({ onMessage, initialFileId }: SenderProps) => {
    ...
    const [expiryMinutes, setExpiryMinutes] = useState(env.ui.defaultExpiryMinutes);
    const [expiryType, setExpiryType] = useState<ExpiryType>(ExpiryType.TimeBased);
    ...
    const handleFileSelect = useCallback(
        (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (file) {
                onMessage(`Selected: ${file.name} (${formatFileSize(file.size)})`, 'info');
            }
        },
        [onMessage],
    );
    ...
    const handleUpload = useCallback(async () => {
        ...
        if (expiryMinutes < env.ui.minExpiryMinutes || expiryMinutes > env.ui.maxExpiryMinutes) {
            onMessage(
        ...
        if (invalid.length > 0) {
            onMessage(`Invalid emails (must be @radixweb.com): ${invalid.join(', ')}`, 'error');
            return;
        }
    ...
    useEffect(() => {
        ...
        if (fileId && uploadedFileId === fileId && autoLoadedForFileId.current !== fileId) {
    ...
    const recipientEmailList = useMemo(() => parseEmails(emailsText), [emailsText, parseEmails]);
    ...
    return (
        <div className="space-y-6">
                    ...
                    {hasFile && (
                        <div className="flex items-center justify-between bg-green-950/40 p-3 rounded-lg border border-green-600/50">
                            <div className="text-sm">
                                <p className="font-semibold text-green-300">{state.file?.name}</p>
            ...
            {/* Recipients */}
            {isEncrypted && (
                <Card title="Step 3: Add Recipients" subtitle="Enter @radixweb.com emails">
                    <div className="space-y-4">
                        <Input
                            type="email"
                            value={emailsText}
