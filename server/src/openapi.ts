    ...
    paths: {
      "/api/health": {
        get: {
          ...
          description: "Returns server health, DB and storage status, and basic stats. No authentication required.",
              ...
              "multipart/form-data": {
                schema: {
                  ...
                  properties: {
                    ...
                    expiryMinutes: { type: "integer", minimum: 5, maximum: 1440, description: "5–1440" },
                    expiryType: { type: "string", enum: ["one-time", "time-based"] },
                    recipientEmail: { type: "string", format: "email", description: "Single recipient (legacy)" },
                    otp: { type: "string", pattern: "^[0-9]{6}$", description: "6-digit OTP (legacy)" },
                    otpHash: { type: "string", description: "Base64 SHA-256 hash of OTP (legacy)" },
                    recipients: {
                      type: "string",
                      description: "JSON string of RecipientPayload[] for multiple recipients",
                    },
                    encryptedData: { type: "string", format: "binary", description: "Encrypted file" },
                    wrappedKey: { type: "string", format: "binary", description: "Wrapped key (single or first)" },
                    wrappedKeySalt: { type: "string", format: "binary", description: "Salt for wrapped key" },
                  },
                },
              },
            },
                    ...
                    allOf: [
                      { $ref: "#/components/schemas/ApiError" },
                      {
                        ...
                        properties: {
                          details: {
                            type: "object",
                            properties: { attemptsRemaining: { type: "number" } },
                          },
                        },
                      },
                    ],
      ...
      "/api/files/{fileId}/recipients": {
        get: {
          ...
          description: "Returns list of recipients for a file (IDs, emails, OTP attempts, timestamps).",
