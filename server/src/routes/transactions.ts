...
router.get(
  ...
  requireAuth,
  queryValidation,
  async (req: Request, res: Response<TransactionsResponse | { error: string; message: string; details?: unknown }>) => {
    ...
    try {
      const opts: { page: number; limit: number; scope?: "all"; type?: "sent" | "received" } = { page, limit };
