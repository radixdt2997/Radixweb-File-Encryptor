...
const uploadValidation = [
  ...
  body('otpHash')
    .matches(/^[A-Za-z0-9+/=]{88}$/)  // SHA-256 base64 is always 88 chars
...
router.post('/', uploadValidation, async (req, res) => {
  ...
  try {
    ...
    if (!req.files || !req.files.encryptedData || !req.files.wrappedKey || !req.files.wrappedKeySalt) {
      return res.status(400).json({
    ...
    setImmediate(async () => {
      try {
        ...
        await sendDownloadLinkEmail(recipientEmail, {
          fileName,
          fileSize: fileResult.size,
          downloadUrl,
          expiryMinutes
        ...
        await logAuditEvent(fileId, 'email_failed', req.ip, req.get('User-Agent'), {
    ...
    res.status(200).json({
      fileId,
      downloadUrl,
      message: 'File uploaded successfully. Download link and OTP sent to recipient.',
      uploadedAt: new Date().toISOString(),
      expiresAt: new Date(Date.now() + expiryMinutes * 60 * 1000).toISOString()
    });

    console.log(`✅ File uploaded: ${fileId} (${fileResult.size} bytes) for ${recipientEmail}`);
