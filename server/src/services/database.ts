...
import { ExpiryType, FileStatus, TransactionRole, UserRole } from '../types/database';
...
export async function getFileRecord(fileId: string): Promise<FileRecord | null> {
    const p = getPool();
    const result = await p.query('SELECT * FROM files WHERE file_id = $1', [fileId]);
...
export async function updateFileStatus(
    ...
    values.push(fileId);
    const p = getPool();
    await p.query(`UPDATE files SET ${updates.join(', ')} WHERE file_id = $${i}`, values);
}
...
export async function updateFileRecord(
    ...
    values.push(fileId);
    const p = getPool();
    await p.query(`UPDATE files SET ${setClause} WHERE file_id = $${keys.length + 1}`, values);
}
...
export async function getRecipientByFileAndEmail(
    ...
    const p = getPool();
    const result = await p.query('SELECT * FROM recipients WHERE file_id = $1 AND email = $2', [
        fileId,
        email,
    ]);
    const row = result.rows[0];
...
export async function updateRecipientRecord(
    ...
    const p = getPool();
    await p.query(`UPDATE recipients SET ${setClause} WHERE id = $${keys.length + 1}`, values);
}

export async function deleteRecipient(fileId: string, recipientId: string): Promise<void> {
    const p = getPool();
    await p.query('DELETE FROM recipients WHERE id = $1 AND file_id = $2', [recipientId, fileId]);
}
